...
const Collection = () => {
  ...
  // Toggle categories or clear all filters
  const handleFilterChange = (category) => {
    setSelectedFilters((prev) => {
      if (Array.isArray(category)) return []; // Clear all
  ...
  const getFilteredAndSortedProducts = () => {
    ...
    if (selectedFilters.length > 0) {
      const lowercasedFilters = selectedFilters.map((f) => f.toLowerCase());
      filtered = filtered.filter(
        (item) =>
          item.category &&
          lowercasedFilters.includes(item.category.toLowerCase())
      );
    }
        ...
        {isFilterOpen && (
          <>
            <motion.div
              ...
              className="fixed top-0 left-0 z-50 w-full max-w-xs h-full bg-white shadow-xl lg:hidden"
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        >
          ...
          <p className="mt-2 text-gray-500">
            Discover our curated selection of fine apparel.
          </p>
        </div>
        ...
        <div className="flex items-start gap-8">
          {/* Desktop Sidebar */}
          <aside
            className="hidden lg:block w-64 sticky top-24 animate-fade-in-up"
          ...
          <div className="flex-1">
            {/* Controls Header */}
            <div
              className="flex justify-between items-center border-b pb-4 mb-6 animate-fade-in-up"
